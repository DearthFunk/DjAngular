angular.module('visualizationModule', [])
    ...
    .service('visualizationProcessingService', function($rootScope, uiThemeService, audioChannelsService,visualizationResizeService, uiService, visualizationThemeService){
        ...
        visualizer.visualization = angular.isObject(djangularLocalStorage) ? djangularLocalStorage.visualization : startingVisualization;




        console.log(djangularLocalStorage.visualization);
        ...
        visualizer.visParticleFountain = function() {
        ...
        visualizer.visParticleRings = function() {
		...
		visualizer.visParticleExplosion = function() {
		...
		function drawHexagon( xPos, yPos, xIndex, yIndex, d, sideLength, fill) {
			hexHeight = Math.sin(hexagonAngle) * sideLength;
			hexRadius = Math.cos(hexagonAngle) * sideLength;
			hexRectangleHeight = sideLength + 2 * hexHeight;
			hexRectangleWidth = 2 * hexRadius;

			if (sideLength > visualizationThemeService.styles.hexMap.minSize) {
				var centerY = hexRectangleHeight/2;
				var centerX = hexRectangleWidth/2;
				visualizer.ctx.beginPath();
				visualizer.ctx.moveTo(xPos + hexRadius - centerX, yPos - centerY);
				visualizer.ctx.lineTo(xPos + hexRectangleWidth - centerX, yPos + hexHeight - centerY);
				visualizer.ctx.lineTo(xPos + hexRectangleWidth - centerX, yPos + hexHeight + sideLength - centerY);
				visualizer.ctx.lineTo(xPos + hexRadius - centerX, yPos + hexRectangleHeight - centerY);
				visualizer.ctx.lineTo(xPos - centerX, yPos + sideLength + hexHeight - centerY);
				visualizer.ctx.lineTo(xPos - centerX, yPos + hexHeight - centerY);
				if(fill) {
					visualizer.ctx.fillStyle=fill;
					visualizer.ctx.fill();
				}
				visualizer.ctx.closePath();
				visualizer.ctx.stroke();
			}
			if (visualizationThemeService.styles.hexMap.showText) {
				visualizer.ctx.font="12px Georgia";
				visualizer.ctx.fillStyle = visualizationThemeService.styles.hexMap.textColor;
				visualizer.ctx.fillText(xIndex + ":" + yIndex,xPos-14,yPos+3);
			}
		}

		visualizer.visHexMap = function() {
			var db = visualizer.getAverageDB();
			var sideLengthMAIN = parseFloat(visualizationThemeService.styles.hexMap.size);
			...
			visualizer.ctx.strokeStyle = hexToRGBA(visualizationThemeService.styles.hexMap.border, visualizationThemeService.styles.hexMap.borderOpacity);
			visualizer.ctx.lineWidth = visualizationThemeService.styles.hexMap.borderWidth;

			for(var x = 0; x * hexRectangleWidthMAIN < visualizationResizeService.canvasInfo.canvasWidth - hexRectangleWidthMAIN; x++) {
				for(var y = 0; y * hexRadiusMAIN < visualizationResizeService.canvasInfo.canvasHeight; y++) {
					var xPos = x * hexRectangleWidthMAIN + ((y % 2) * hexRadiusMAIN);
					var yPos = y * (sideLengthMAIN + hexHeightMAIN);
					var d = Math.sqrt( Math.pow(visualizationResizeService.canvasInfo.canvasWidthHalf - xPos, 2) + Math.pow(visualizationResizeService.canvasInfo.canvasHeightHalf - yPos, 2) );

					if (Math.pow(xPos-visualizationResizeService.canvasInfo.canvasWidthHalf,2) +
						Math.pow(yPos-visualizationResizeService.canvasInfo.canvasHeightHalf,2) < Math.pow(db*visualizationThemeService.styles.hexMap.distance,2) ) {
						drawHexagon(
							xPos,
							yPos,
							x,
							...
							d,
							(sideLengthMAIN *
							(db * visualizationThemeService.styles.hexMap.dbImpact) *
							(1-(d/visualizationThemeService.styles.hexMap.stretch)) ),

							hexToRGBA(visualizationThemeService.styles.hexMap.fill, 1- (d / visualizationThemeService.styles.hexMap.fillOpacity ) )
						);
					}
				}
			}
		...
		flare.src = "img/flare3.png";
		var percent2;
		var padding = 2;
		var crossover = 4.5;
		var bottomOffset = 100;
		visualizer.visSunAndClouds = function() {
			var db = visualizer.getAverageDB();
			var freqArray = visualizer.getFreqArray();
			var timeArray = visualizer.getTimeArray();

			var minSunSize = visualizationThemeService.styles.sunAndClouds.minSunSize;
			var cloudGrowth = visualizationThemeService.styles.sunAndClouds.cloudGrowth;
			var sunGrowth = visualizationThemeService.styles.sunAndClouds.sunGrowth;

			//---------------------LENS FLARE
			if (db > 0) {
				visualizer.ctx.drawImage(
					flare,
					visualizationResizeService.canvasInfo.canvasWidthHalf - (flare.width/2),
					visualizationResizeService.canvasInfo.canvasHeightHalf - (flare.height/2));
			}
			...
			if (db>0) {
				visualizer.ctx.beginPath();
				visualizer.ctx.fillStyle = visualizationThemeService.styles.sunAndClouds.sunColor;
				visualizer.ctx.arc(centerSun.x, centerSun.y, centerSun.r, 0, 2 * Math.PI, true);
				visualizer.ctx.fill();
				visualizer.ctx.closePath();
			}
			//--------------------SCOPE
			if (visualizationThemeService.styles.sunAndClouds.showScope && db>0) {
				...
				for (index = 0; index < timeArray.length; index++) {
					...
					index == 0 ?
						offset1 = visualizationResizeService.canvasInfo.canvasHeightHalf :
						offset1 = visualizationResizeService.canvasInfo.canvasHeightHalf + (centerSun.r*2*(percent1-0.5));
					index == timeArray.length-1 ?
						offset2 = visualizationResizeService.canvasInfo.canvasHeightHalf :
						offset2 = visualizationResizeService.canvasInfo.canvasHeightHalf + (centerSun.r*2*(percent2-0.5));

					visualizer.ctx.beginPath();
					visualizer.ctx.lineWidth = 4;
					visualizer.ctx.strokeStyle = visualizationThemeService.styles.sunAndClouds.scopeColor;
					visualizer.ctx.moveTo(index*barWidth + padding +(visualizationResizeService.canvasInfo.canvasWidthHalf-centerSun.r)           ,offset1);
					visualizer.ctx.lineTo(index*barWidth + padding +(visualizationResizeService.canvasInfo.canvasWidthHalf-centerSun.r) + barWidth,offset2);
					visualizer.ctx.stroke();
					visualizer.ctx.closePath();
				}
			}
			...
			for (var i = 0; i < freqArray.length; i++) {
				visualizer.ctx.beginPath();
				visualizer.ctx.fillStyle = visualizationThemeService.styles.sunAndClouds.techniClouds ? randomHex() : visualizationThemeService.styles.sunAndClouds.cloudColor;

				//left
				visualizer.ctx.arc(
					i * (visualizationResizeService.canvasInfo.chunkWidth * crossover),
					visualizationResizeService.canvasInfo.canvasHeight + bottomOffset - freqArray[i],
					freqArray[i] * cloudGrowth,
					0, 2 * Math.PI, true);
				//right
				visualizer.ctx.arc(
					visualizationResizeService.canvasInfo.canvasWidth - (i * (visualizationResizeService.canvasInfo.chunkWidth * crossover)),
					visualizationResizeService.canvasInfo.canvasHeight + bottomOffset - freqArray[i],
					freqArray[i] * cloudGrowth,
					0, 2 * Math.PI, true);
